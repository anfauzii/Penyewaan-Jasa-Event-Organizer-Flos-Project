...
namespace FlosProjects
{
    public partial class DataBarang : Form
    {
        ...
        void Search()
        {
            ...
            {
                Koneksi.conn.Open();
                MySqlDataAdapter Tampil = new MySqlDataAdapter("SELECT  * FROM barang where  id_barang like '%" + txtSearch.Text + "%' or nama_barang like '%" + txtSearch.Text + "%'", Koneksi.conn);
                DataTable data = new DataTable();
                Tampil.SelectCommand.ExecuteNonQuery();
                Tampil.Fill(data);
                dgvBarang.DataSource = data;
        ...
        private void btnSimpan_Click(object sender, EventArgs e)
        {
            if (txtIdBarang.Text.Trim() == "" || txtNama.Text.Trim() == "" || txtHarga.Text.Trim() == "" ||
                txtStok.Text.Trim() == "" || cbKategori.Text.Trim() == "")
            {
            ...
            else
            {
                ...
                {
                    Koneksi.conn.Open();
                    cmd = new MySqlCommand("insert into barang(id_barang,nama_barang,harga,stok,nama_kategori) values('" + txtIdBarang.Text + "', '" + txtNama.Text + "', '" + txtHarga.Text + "', '" + txtStok.Text + "', '" + cbKategori.Text + "')", Koneksi.conn); ;
                    cmd.ExecuteNonQuery();
        ...
        {
            if (txtIdBarang.Text.Trim() == "" || txtNama.Text.Trim() == "" || txtHarga.Text.Trim() == "" || txtStok.Text.Trim() == "" || cbKategori.Text.Trim() == "" )
            {
                MessageBox.Show("Data Tidak Lengkap", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                try
                {
                    cmd = new MySqlCommand("update barang set nama_barang='" + txtNama.Text + "', harga='" + txtHarga.Text + "', stok='" + txtStok.Text + "', nama_kategori='" + cbKategori.Text + "' where id_barang='" + txtIdBarang.Text + "'", Koneksi.conn);
                    Koneksi.conn.Open();
                    cmd.ExecuteNonQuery();
