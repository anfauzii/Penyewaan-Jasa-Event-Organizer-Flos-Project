    ...
    public partial class DataUser : Form
    {
        ...
        void ShowData()
        {
            try
            {
                ...
                ("SELECT id_user,nama,email,telepon,level_user,status FROM users", Koneksi.conn);
        ...
        void Search()
        {
            try
            {
                Koneksi.conn.Open();
                MySqlDataAdapter Tampil = new MySqlDataAdapter
                    ("SELECT  id_user,nama,email,telepon,level_user,status FROM users where id_user like '%" + txtSearch.Text + "%' or nama like '%" + txtSearch.Text + "%' or telepon like '%" + txtSearch.Text + "%'", Koneksi.conn);
        ...
        void IdOtomatis()
        {
            ...
            cmd = new MySqlCommand("select id_user from users where id_user in(select max(id_user) from users) order by id_user desc", Koneksi.conn);
        ...
        private void txtSimpan_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    cmd = new MySqlCommand("insert into users(id_user,nama,email,telepon,passwd,level_user,status) values('" + txtIdUser.Text + "', '" + txtNama.Text + "', '" + txtEmail.Text + "', '" + txtNoHP.Text + "', '" + txtPasswd.Text + "', '" + cbLevel.Text + "', '" + cbStatus.Text + "')", Koneksi.conn);
                    Koneksi.conn.Open();
                    cmd.ExecuteNonQuery();
        ...
        private void txtEdit_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    cmd = new MySqlCommand("update users set nama='"+ txtNama.Text + "', email='" + txtEmail.Text + "', telepon='" + txtNoHP.Text + "', passwd='" + txtPasswd.Text + "', level_user='" + cbLevel.Text + "', status='" + cbStatus.Text + "' where id_user='" + txtIdUser.Text + "'", Koneksi.conn);
                    Koneksi.conn.Open();
                    cmd.ExecuteNonQuery();
        ...
        private void txtDelete_Click(object sender, EventArgs e)
        {
                ...
                {
                    cmd = new MySqlCommand("delete from users where id_user='" + txtIdUser.Text + "'", Koneksi.conn);
                    Koneksi.conn.Open();
                    cmd.ExecuteNonQuery();
